...
{	
	...
	public function getTickets($realm)
	{
		if($realm)
		{
			...
			//Do the query
			if(column("gm_ticket", "closedBy", $realm->getId()))
			{
				$query = $realm->getCharacters()->getConnection()->query("SELECT ".allColumns("gm_ticket", $realm->getId())." FROM ".table("gm_tickets", $realm->getId())." WHERE ".column("gm_ticket", "completed", false, $realm->getId())." = 0 AND ".column("gm_ticket", "closedBy", false, $realm->getId())." = 0");
			}
			elseif(column("gm_ticket", "completed", $realm->getId()))
			{
				$query = $realm->getCharacters()->getConnection()->query("SELECT ".allColumns("gm_ticket", $realm->getId())." FROM ".table("gm_tickets", $realm->getId())." WHERE ".column("gm_ticket", "completed", false, $realm->getId())." = 0");
			}
			else
			{
				$query = $realm->getCharacters()->getConnection()->query("SELECT ".allColumns("gm_ticket", $realm->getId())." FROM ".table("gm_tickets", $realm->getId()));
			}
	...
	public function getTicket($realm, $ticketId = false)
	{
		if($ticketId && $realm)
		{
			...
			//Do the query
			$query = $realm->getCharacters()->getConnection()->query("SELECT ".allColumns("gm_ticket", $realm->getId())." FROM ".table("gm_ticket", $realm->getId())." WHERE ".column("gm_ticket", "Id", false, $realm->getId())." = ?", array($ticketId));
	...
	public function deleteTicket($realmConnection, $ticketId, $realmId)
	{
		...
		{
			$realmConnection->query("DELETE FROM ".table("gm_ticket", $realmId)." WHERE ".column("gm_ticket", "Id", false, $realmId)." = ?",array($ticketId));
	...
	public function setTicketCompleted($realmConnection, $ticketId, $realmId)
	{
		if($ticketId && $realmConnection)
		{
			...
			{
				$realmConnection->query("UPDATE ".table("gm_ticket", $realmId)." SET ".column("gm_tickets", "completed", false, $realmId)." = 1, ".column("gm_ticket", "closedBy", false, $realmId)."=".column("gm_ticket", "guid", false, $realmId)." WHERE ".column("gm_tickets", "Id", false, $realmId)." = ?",array($ticketId));
			}
			else
			{
				$realmConnection->query("UPDATE ".table("gm_ticket")." SET ".column("gm_tickets", "completed", false, $realmId)." = 1 WHERE ".column("gm_ticket", "Id", false, $realmId)." = ?",array($ticketId));
			}
